...
public class LeaveRequestLeaveBalanceIntegrationTests {
    ...
    void testLeaveCancelEvents() throws LeaveRequestDomainException {
        Identity leaveId = new Identity("leave002");
        Identity balanceId = new Identity("bal002");
        UserJpa user = new UserJpa();

        LeaveBalance balance = LeaveBalance.leaveBalanceOf(
                balanceId, "staff1234", "2025", INITIAL_BALANCE - 7.5f
        );
        ...
        assertEquals("No longer needed", leaveCancelledEvent.getReason());
        ...
        assertEquals("staff1234", balanceAddEvent.getStaffId());
        assertEquals("2025", balanceAddEvent.getLeaveYear());
        assertEquals(INITIAL_BALANCE, balanceAddEvent.getNewBalance());
    }
}
